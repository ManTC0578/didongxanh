...
public class CsvReader {
    ...
    /**
     * Creates a {@link t using a file
     * as the data source.&nbsp;Uses UTF-8 as the
     ...
     * @Param charset
     */
    public CsvReader(String fileName, String charset) throws FileNotFoundException {
        this(fileName, Letters.COMMA, Charset.forName(charset));
    }

    /**
     * Constructs a {@link t using a
    ...
    public String get(int columnIndex) throws IOException {
        ...
        if (columnIndex > -1 && columnIndex < columnsCount) {
            return convertCharset(values[columnIndex]);
        } else {
            return "";
        }
    }
        ...
        if (!initialized) {
            if (fileName != null) {
                inputStream = new BufferedReader(new InputStreamReader(
                        new FileInputStream(fileName), DEFAULT_CHARSET),
    ...
    private String convertCharset(String org) {
        if(charset == null)
            return org;

        byte[] strBytes = org.getBytes(charset);
